...
nets = dict(
    ...
    zeitcoin=math.Object(
        P2P_PREFIX='ced5dbfa'.decode('hex'),
        P2P_PORT=44845,
        ADDRESS_VERSION=51,
        RPC_PORT=44843,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'Zeitcoinaddress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
	SUBSIDY_FUNC=lambda target: get_subsidy(6, 100, target),
	...
	BLOCKHASH_FUNC=lambda data: pack.IntType(256).unpack(__import__('ltc_scrypt').getPoWHash(data)),
        ...
        SYMBOL='ZEIT',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'zeitcoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/zeitcoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.zeitcoin'), 'zeitcoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://blockexplorer.zeit-coin.com/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://blockexplorer.zeit-coin.com/address/',
        TX_EXPLORER_URL_PREFIX='http://blockexplorer.zeit-coin.com/transaction/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
